...
const viewEmployees = async () => {
  ...
  const response = await db
    ...
    ); //issue: couldn't get manager name to replace the manager id, solved by renaming employee table as employee manager and then used left join on empployee manager on manager.id = employee.manager_id
...
const addEmployee = async () => {
  ...
  const newEmployee =
    ...
    console.log(
      `'${data.employeeLastName}, ${data.employeeFirstName}' has been added to employee database.`
    ));
  app();
};
...
const updateEmployeeRole = async () => {
  ...
  const roleChoices = rolesData[0].map((role) => {
    return { name: role.title, value: role.id };
  });
  ...
  const updatedEmployeeRole =
    ...
    console.log(
      `Employee with ID: ${data.employeeName} role updated successfully`
    ));
  app();
};
...
const viewRoles = async () => {
  console.log("========== Roles ==========");
  const rolesTable = await db.promise().query("select id, title from role;");
...
const addRole = async () => {
  ...
  const newRole =
    ...
    console.log(`Role '${data.newRoleTitle} has been added!'`));
  app();
};

const viewDepartments = async () => {
  console.log("========== Departments ==========");
  const departmentTable = await db.promise().query("select * from department;");
...
const addDepartment = async () => {
  ...
  const newDepartment =
    (db.query(
      `insert into department(name) values ('${data.newDepartmentName}');`
    ),
    console.log(`Department ${data.newDepartmentName} has been added!`));
  app();
};
